...
#include <base_local_planner_test/angles.h>
...
bool LatchedStopRotateController::isPositionReached(LocalPlannerUtil* planner_util,
  ...
  if ((latch_xy_goal_tolerance_ && xy_tolerance_latch_) ||
      base_local_planner_test::getGoalPositionDistance(global_pose, goal_x, goal_y) <= xy_goal_tolerance) {
...
bool LatchedStopRotateController::isGoalReached(LocalPlannerUtil* planner_util,
  ...
  base_local_planner_test::LocalPlannerLimits limits = planner_util->getCurrentLimits();
  ...
  if ((latch_xy_goal_tolerance_ && xy_tolerance_latch_) ||
    ...
    double goal_th = base_local_planner_test::getYaw(goal_pose.pose.orientation);
    double angle = base_local_planner_test::getGoalOrientationAngleDifference(global_pose, goal_th);
    //check to see if the goal orientation has been reached
    if (fabs(angle) <= limits.yaw_goal_tolerance) {
      //make sure that we're actually stopped before returning success
      if (base_local_planner_test::stopped(base_odom, rot_stopped_vel, trans_stopped_vel)) {
        return true;
      }
    }
  }
...
bool LatchedStopRotateController::stopWithAccLimits(const arc::PoseStamped& global_pose,
    const arc::PoseStamped& robot_vel,
    arc::Twist& cmd_vel,
  ...
  double vel_yaw =base_local_planner_test::getYaw(robot_vel.pose.orientation);
  double vth = sign(vel_yaw) * std::max(0.0, (fabs(vel_yaw) - acc_lim[2] * sim_period));

  //we do want to check whether or not the command is valid
  double yaw =base_local_planner_test::getYaw(global_pose.pose.orientation);
...
bool LatchedStopRotateController::rotateToGoal(
    ...
    Eigen::Vector3f acc_lim,
    double sim_period,
    base_local_planner_test::LocalPlannerLimits& limits,
    boost::function<bool (Eigen::Vector3f pos,
                          Eigen::Vector3f vel,
                          Eigen::Vector3f vel_samples)> obstacle_check) {
  double yaw = base_local_planner_test::getYaw(global_pose.pose.orientation);
  ...
  if (valid_cmd) {
    ...
    //printf("dwa_local_planner", "Moving to desired goal orientation, th cmd: %.2f, valid_cmd: %d\n", v_theta_samp, valid_cmd);
...
bool LatchedStopRotateController::computeVelocityCommandsStopRotate(arc::Twist& cmd_vel,
    Eigen::Vector3f acc_lim,
    double sim_period,
  ...
  if (fabs(angle) <= limits.yaw_goal_tolerance) {
   ...
   // ROS_DEBUG("Angle: %f Tolerance: %f", angle, limits.yaw_goal_tolerance);
    ...
    if ( ! rotating_to_goal_ && !base_local_planner_test::stopped(base_odom, limits.rot_stopped_vel, limits.trans_stopped_vel)) {
