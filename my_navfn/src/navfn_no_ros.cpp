...
namespace navfn {
  ...
  NavfnNoROS::NavfnNoROS(costmap_2d_test::Costmap2D_Test* costmap)
    : costmap_(NULL),  planner_(),  allow_unknown_(true) {
      //initialize the planner
      initializeNoRos(costmap);
  }
  
void NavfnNoROS::initializeNoRos(costmap_2d_test::Costmap2D_Test* costmap){
      costmap_=costmap;     
  ...
  bool NavfnNoROS::makePlanNoRos(const arc::PoseStamped& start,
      const arc::PoseStamped& goal, double tolerance, std::vector<arc::PoseStamped>& plan){
...
/////////////////////////////////////////////// ZACH DEBUG END
    ...
    costmap_->setCost(mx,my,costmap_2d_test::FREE_SPACE);
...
;
    ...
    if(found_legal){
      ...
      if(getPlanFromPotentialNoRos(best_pose, plan)){
        arc::PoseStamped goal_copy = best_pose;
...
////////////// ZACH DEBUG END
  ...
  bool NavfnNoROS::getPlanFromPotentialNoRos(const arc::PoseStamped& goal, std::vector<arc::PoseStamped>& plan){
